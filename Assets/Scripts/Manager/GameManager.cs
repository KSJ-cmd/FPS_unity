...
public class GameManager : MonoBehaviour
{
    ...
    public GameObject [] spawnPosition = new GameObject[4];
    ...
    void Update()
    {
        if (!isGameover)
        {
            ...
            { 
                foreach(GameObject enemyPosition in spawnPosition)
                {
                    GameObject enemy = Instantiate(bullerSpawnerPrefab, enemyPosition.transform.position, Quaternion.identity);
                    enemy.transform.parent = level.transform;
                    enemy.transform.localPosition = enemyPosition.transform.position;
                    spawnerList.Add(enemy);
                }
            }
            prevTime = currTime;
        }
